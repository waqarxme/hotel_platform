...
export default function OwnerReviewsPage() {
  ...
  if (isLoading) {
    return <div className="py-20 text-center text-xs text-slate-500">Loading guest reviews...</div>;
  }
  ...
  return (
    <div className="space-y-8 text-slate-900">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-slate-950 font-heading">Customer Reviews & Responses</h1>
        ...
        <div className="flex items-center gap-3 p-3 rounded-2xl bg-white border border-slate-200 shadow-sm">
          <div className="flex items-center gap-1 text-amber-500">
            ...
            <span className="text-xl font-bold text-slate-950 font-mono">{avgRating}</span>
          </div>
      ...
      {reviews.length === 0 ? (
        <div className="bg-white rounded-3xl p-12 text-center text-slate-500 text-xs border border-slate-200 shadow-sm">
          No guest reviews published yet.
        </div>
      ) : (
        <div className="space-y-4">
          {reviews.map((rev) => (
            <div key={rev.id} className="bg-white rounded-3xl p-6 space-y-4 border border-slate-200 shadow-sm">
              <div className="flex items-start justify-between">
                <div>
                  <h3 className="text-sm font-bold text-slate-950">{rev.guestName}</h3>
              ...
              {rev.response ? (
                <div className="p-4 rounded-2xl bg-red-50/50 border-l-2 border-red-600 space-y-1">
                  <div className="flex items-center justify-between text-[11px]">
                    <span className="font-bold text-red-700 uppercase tracking-wider">
                      Official Management Reply
                    </span>
                    <span className="text-slate-400 font-mono">{formatDate(rev.respondedAt || "")}</span>
                  </div>
