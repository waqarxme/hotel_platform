...
export default function AdminCleaningPage() {
  ...
  if (isLoading) {
    return <div className="py-20 text-center text-xs text-slate-400">Loading cleaning operations...</div>;
  }
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="space-y-4">
        <h2 className="text-base font-bold text-white font-heading">Available Cleaning Squads ({teams.length})</h2>
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          {teams.map((team) => (
            <div key={team.id} className="glass-card rounded-xl p-5 space-y-3 border border-cobalt-800">
              <div className="flex items-center justify-between">
                <span className="p-2 rounded-lg bg-cobalt-500/10 text-cobalt-400 border border-cobalt-500/20">
              ...
              <div>
                ...
                <p className="text-[11px] text-slate-400">{team.contactPhone}</p>
              </div>

              <div className="pt-2 border-t border-cobalt-800 text-[11px] text-slate-300 flex items-center justify-between">
                <span>Active Assignments:</span>
                <span className="font-mono font-bold text-white">{team.activeAssignments}</span>
              </div>
            </div>
          ))}
        </div>
      ...
      <div className="glass-panel rounded-2xl p-6 space-y-4">
        <h2 className="text-base font-bold text-white font-heading">Hotel Cleaning Requests</h2>
              ...
              <tbody className="divide-y divide-cobalt-800/40">
                {requests.map((r) => (
                  <tr key={r.id} className="hover:bg-cobalt-900/40">
                    <td className="px-4 py-3.5 font-bold text-white">{r.hotelName}</td>
                    <td className="px-4 py-3.5 text-slate-300">{r.roomNumbers}</td>
                    <td className="px-4 py-3.5 text-slate-400">{formatDate(r.requestedDate)}</td>
                    <td className="px-4 py-3.5">
                      {r.assignedTeamName ? (
                        <span className="text-cobalt-300 font-semibold">{r.assignedTeamName}</span>
                      ) : (
